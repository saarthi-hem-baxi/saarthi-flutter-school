...
class PendingTestsPage extends StatefulWidget {
  const PendingTestsPage({
    Key? key,
  }) : super(key: key);

  @override
  State<PendingTestsPage> createState() => _PendingTestsPageState();
}

class _PendingTestsPageState extends State<PendingTestsPage> with SingleTickerProviderStateMixin {
  ...
  _loadMore() async {
    bool issuccess = await testsController.getPendingGeneralTests(context: context, page: currentPage.toString(), limit: "15");
    if (issuccess) {
      if ((testsController.testsPendingModel.value.tests ?? []).isNotEmpty) {
        currentPage += 1;
        tests?.addAll(testsController.testsPendingModel.value.tests!);
        shouldExecute = true;
        if (mounted) setState(() {});
