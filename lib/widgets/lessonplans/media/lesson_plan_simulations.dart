...
class LessonPlanSimulations extends StatelessWidget {
  const LessonPlanSimulations({
    Key? key,
  ...
  bool isShowMedia(LessonPlanItem simulation) {
    if (type == LPQBType.topic) {
      if (lessonPlan?.isPublisher == true) {
        int index = (simulation.topics ?? []).indexWhere((element) => element.topic == id);
        if (index != -1) {
          return true;
        }
      } else {
        //for when lesson plan is saarthi's lesson plan
        for (var concept in simulation.concepts ?? []) {
      ...
      bool isContain = (simulation.concepts ?? []).contains(id);
      return isContain;
    }
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      child: (lessonPlan?.simulations ?? []).isNotEmpty
          ? Row(
              children: getData(lessonPlan?.simulations ?? []).map(
                (lessonplanssimulations) {
                  if (isShowMedia(lessonplanssimulations)) {
                    return Stack(
                      children: [
                        AppMediaWidget(
                          mediaUrl: lessonplanssimulations.url?[lang] ?? "",
                          title: lessonplanssimulations.name?[lang] ?? "",
                                  ...
                                  builder: (context) => ContentReportPage(
                                    ...
                                    lpContentReportdata: LpContentReportModal(
                                      ...
                                      content: LPReportContent(
                                        ...
                                        simulation: LPReportItem(contentId: lessonplanssimulations.id ?? ""),
                                      ),
                                    ),
                                  ),
                                ),
