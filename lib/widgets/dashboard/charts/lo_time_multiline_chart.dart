...
import '../../../model/dashboard/lo_multi_line_week.dart';
import '../../../theme/colors.dart';
import '../../../theme/style.dart';
import '../multi_line_chart_year.dart';

class LOTimeMultiLineChart extends StatefulWidget {
  const LOTimeMultiLineChart({Key? key}) : super(key: key);

  @override
  State<LOTimeMultiLineChart> createState() => _LOTimeMultiLineChartState();
}

class _LOTimeMultiLineChartState extends State<LOTimeMultiLineChart> {
  ...
  void _prepareChartData() {
    ...
    // _dashboardController.getLOMultiLineChartWeekWise().then((value) {
    //   for (var item in _dashboardController.loMultiLineWeekData.value.chartData ?? []) {
    //     int index = _dashboardController.loMultiLineWeekData.value.chartData?.indexOf(item) ?? 0;
    ...
    // _dashboardController.getLOMultiLineChartQuaterWise().then((value) {
    //   for (var item in _dashboardController.loMultiLineQuaterData.value.chartData ?? []) {
    //     int index = _dashboardController.loMultiLineQuaterData.value.chartData?.indexOf(item) ?? 0;
    //     MultiLineChartDataModal d = MultiLineChartDataModal(
    //       lineIndex: index,
    //       lineColor: getLineColor(index),
    //       isChecked: true,
    ...
    // _dashboardController.getLOMultiLineChartMonthWise().then((value) {
    //   for (var item in _dashboardController.loMultiLineMonthData.value.chartData ?? []) {
    //     int index = _dashboardController.loMultiLineMonthData.value.chartData?.indexOf(item) ?? 0;
    //     MultiLineChartDataModal d = MultiLineChartDataModal(
    //       lineIndex: index,
    //       lineColor: getLineColor(index),
    //       isChecked: true,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Obx(() {
        if (_dashboardController.loMultiChartLoading.isTrue || isPrepararingData == true) {
          return const Align(
