...
import 'package:saarthi_pedagogy_studentapp/screen/auth/license_expire_popup.dart';
...
class HeaderHandler {
  ...
  void handleHeaders(response) async {
    ...
    if (resHeader['APP_NAME_REQUIRED'] != null && resHeader['APP_NAME_REQUIRED']![0] == 'YES') {
      ...
      if (_authController.isOpenSubscriptionPopup.value == false) {
        LicenceExpiredPopUp.showPopUp();
        _authController.isOpenSubscriptionPopup.value = true;
      }
  ...
  Future<String> getUpdatedUserAt() async {
    ...
    String currentUserString = prefs.getString(loginUserDataKey) ?? "";
    User? user;
    if (currentUserString != "") {
      user = User.fromJson(jsonDecode(currentUserString));
    }
  ...
  void handleUpdateAvalable(header) async {
    ...
    if (headers['new_version_desc'] != null && headers['new_version_desc']!.isNotEmpty) {
      ...
      Get.defaultDialog(
        ...
        content: Column(
          children: [
            ...
            whatsNewMessage.isNotEmpty ? Text("What's New : $whatsNewMessage") : Container(),
            SizedBox(
              height: 20.h,
            ),
            Row(
